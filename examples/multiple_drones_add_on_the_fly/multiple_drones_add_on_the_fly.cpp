...
int main(int argc, char* argv[])
{
    ...
    std::cout << "cool" << mavsdk.systems().size() << std::endl;
    ...
    sleep_for(seconds(5));
    std::thread t_2(&takeoff_and_land, std::ref(mavsdk.systems().back()));
    threads.push_back(std::move(t_2));
    sleep_for(seconds(5));
...
void takeoff_and_land(std::shared_ptr<System> system)
{
    ...
    telemetry.subscribe_position([system](Telemetry::Position position) {
        std::cout << "drone id: "<< int(system->get_system_id()) << " Altitude: " << position.relative_altitude_m << " m\n";
    });
